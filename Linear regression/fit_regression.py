...
if len(df) <= FORECAST_DAYS:
    raise ValueError(
        f"Not enough rows ({len(df)}) for a {FORECAST_DAYS}-day forecast. "
        f"Need at least {FORECAST_DAYS + 1} rows, ideally much more."
    )
...
future_dates = pd.bdate_range(start=pd.to_datetime(last_date) + pd.Timedelta(days=1), periods=forecast_out)
